...
export const TournamentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const saveTieResult = async (tie: TournamentTie) => {
    const convertedMatches: TournamentMatch[] = tie.matches.map((cm, idx) => {
      ...
      // Winner automatically calculated from Set 1 score (or manual override)
      let winnerId = cm.winner_team_id;
      if (s1 > s2) winnerId = tie.team1_id;
      else if (s2 > s1) winnerId = tie.team2_id;

      const scoreSumm = `${s1} - ${s2}`;
      const isT1Trump = Boolean(cm.team1_trump);
      const isT2Trump = Boolean(cm.team2_trump);
  ...
  const saveMatchResult = async (matchData: TournamentMatch) => {
    ...
    const isT1Trump = Boolean(matchData.team1_trump);
    const isT2Trump = Boolean(matchData.team2_trump);
    ...
    logAuditAction('MATCH_RESULT_ENTERED', { matchNumber: enrichedMatch.match_number, winner: winnerId, trump: enrichedMatch.is_trump_match });
  };
  ...
  const recalculateStandings = (allMatches: TournamentMatch[]) => {
    ...
    const standaloneMatches: TournamentMatch[] = [];
    ...
    allMatches.forEach(m => {
      if (m.status === 'COMPLETED' && m.winner_team_id) {
        ...
        if (m.tie_id) {
          ...
          // If no explicit tie_id, group by composite key
          if (!fixtures[tieKey]) fixtures[tieKey] = [];
          fixtures[tieKey].push(m);
        }
      }
    ...
    // 5 wins -> 6 pts
    // 6 wins -> 7 pts
    const getBasePoints = (wins: number): number => {
      if (wins === 1) return 1;
      if (wins === 2) return 2;
      if (wins === 3) return 3;
      if (wins === 4) return 5;
    ...
    // Calculate standings for each tie fixture
    Object.values(fixtures).forEach(matchList => {
      ...
      matchList.forEach(m => {
        ...
        if (stats[m.team1_id]) {
          stats[m.team1_id].score_for += s1;
          stats[m.team1_id].score_against += s2;
        }
        if (stats[m.team2_id]) {
          stats[m.team2_id].score_for += s2;
          stats[m.team2_id].score_against += s1;
        }
      });
      ...
      if (teamList.length >= 2) {
        ...
        // Trump Bonus Points (+2 bonus points per Trump win, +4 if dual trump)
        let t1TrumpBonus = 0;
        let t2TrumpBonus = 0;
        ...
        // Total Points = Base Points + Trump Bonus Points
        ...
        if (stats[tid]) {
          ...
          stats[tid].points += getBasePoints(wins);
        }
      }
    });
